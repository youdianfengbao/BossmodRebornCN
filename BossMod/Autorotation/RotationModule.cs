...
{
    [PropertyDisplay("开发中 - 预期会出现很多问题（或直接无法使用）")]
    WIP,

    [PropertyDisplay("基础 - 预期标准循环可以工作，但仅此而已 - 适合升级或休闲内容，预期绿色/蓝色日志")]
    Basic,

    [PropertyDisplay("良好 - 预期在大多数情况下执行合理的动作，从死亡中恢复，有基本的规划支持 - 适合零式，预期蓝色/紫色日志")]
    Ok,
...
public enum RotationModuleOrder
{
    ...
    [PropertyDisplay("[2] 标准循环/实用模块。负责决定使用哪些动作并设置目标区域。")]
    Actions = 2,

    [PropertyDisplay("[3] 移动模块。负责路径查找和执行移动。")]
    Movement = 3,
}
